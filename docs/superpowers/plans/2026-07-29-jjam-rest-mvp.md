...
| 결함 | 증상 | 수정 |
|---|---|---|
| 배경음이 10초 늦게 시작 | 16편 전부 `ready` 단계(10초)에 `sound: false` 가 박혀 있어, 시작을 눌러도 배경음은 본체 첫 단계부터 났다. 가장 산만한 첫 10초가 무음이었다 | `ready` 단계의 `sound` 를 전부 `true` 로. `JjamSound.play` 가 1.6초 페이드 인이라 급하게 들이치지 않는다 |
